...
function Field({
  ...
  return (
    // biome-ignore lint/a11y/useSemanticElements: This layout primitive may be nested inside a fieldset.
    <div
      role="group"
      data-slot="field"
  ...
  const content = useMemo(() => {
    ...
    return (
      <ul className="ml-4 flex list-disc flex-col gap-1">
        {uniqueErrors.map(
          (error) => error?.message && <li key={String(error.message)}>{error.message}</li>,
        )}
      </ul>
    )
  }, [children, errors])
